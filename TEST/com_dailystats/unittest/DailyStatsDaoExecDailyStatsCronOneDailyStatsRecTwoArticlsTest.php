...
class DailyStatsDaoExecDailyStatsCronOneDailyStatsRecTwoArticlesTest extends DailyStatsCronTestBase {
	...
	public function testExecDailyStatsCronOneDailyStatsRecTwoArticlesOneDayInterval() {
		...
		$this->assertEquals(4,$count,'2 daily_stats records expected, one for yesterday and one for today');
	...
	public function testExecDailyStatsCronOneDailyStatsRecTwoArticles20DaysInterval() {
		...
		$this->checkEntryExistInLog("Stats for $today added in DB. 0 rows inserted for new attachment\(s\). 2 rows inserted for existing attachments \(gap filled: 20 day\(s\)\).");
	}
